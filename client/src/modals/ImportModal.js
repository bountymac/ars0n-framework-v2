import { Modal, Button, Form, Spinner, Alert, Card, Badge } from 'react-bootstrap';
import { useState, useRef } from 'react';
import { FaUpload, FaFileImport, FaDatabase, FaCheckCircle, FaExclamationTriangle } from 'react-icons/fa';

function ImportModal({ show, handleClose }) {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}:${process.env.REACT_APP_SERVER_PORT}/api/database-import`, {
        method: 'POST',
        body: formData
      });
